...
export function NewReportPage() {
  ...
  const create = useMutation({
    ...
    onSuccess: (report) => navigate(`/expenses/${report.id}`),
  });
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-2xl font-bold">경비 신청</h1>
      ...
      <div className="card flex items-center justify-between">
        ...
        <button className="btn-primary" disabled={!canSubmit || create.isPending} onClick={() => create.mutate()}>
          {create.isPending ? "저장 중…" : "신청서 저장"}
        </button>
      </div>
      {create.isError && <p className="text-sm text-seal">저장에 실패했습니다. 입력값을 확인해 주세요.</p>}
    </div>
  );
}
